...
export default async function InsightPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <>
      {/* Header */}
      <section className="bg-[#09041A] px-[clamp(24px,5vw,80px)] pt-40 pb-16">
        <div className="max-w-[800px] mx-auto">
          <div className="flex items-center gap-3 mb-8">
            <Link href="/insights" className="font-mono text-[10px] tracking-[0.16em] uppercase text-[rgba(242,238,255,0.40)] hover:text-[#C084FC] transition-colors">
              ← Insights
            </Link>
            <span className="text-[rgba(242,238,255,0.20)]">/</span>
          ...
          <h1 className="font-serif font-bold text-[#F2EEFF] leading-tight mb-6"
            style={{ fontSize: 'clamp(32px,5vw,64px)' }}>{ins.title}</h1>
          <div className="flex items-center gap-4">
            <span className="font-mono text-[10px] tracking-[0.1em] text-[rgba(242,238,255,0.35)]">{formatDate(ins.publishedAt)}</span>
            <span className="text-[rgba(242,238,255,0.15)]">·</span>
            <span className="font-mono text-[10px] tracking-[0.1em] text-[rgba(242,238,255,0.35)]">{ins.readTime}</span>
            <span className="text-[rgba(242,238,255,0.15)]">·</span>
            <span className="font-mono text-[10px] tracking-[0.1em] text-[rgba(242,238,255,0.35)]">Apex Growth</span>
          </div>
        </div>
      </section>

      {/* Body */}
      <section className="bg-[#09041A] px-[clamp(24px,5vw,80px)] pb-24">
        <div className="max-w-[800px] mx-auto">
          <div className="border-t border-[rgba(192,132,252,0.10)] pt-12">
            <div className="prose-custom space-y-6">
              {bodyText.split('\n\n').map((para, i) => {
                if (para.startsWith('**') && para.endsWith('**')) {
                  return <h2 key={i} className="font-serif text-[24px] font-bold text-[#F2EEFF] mt-10">{para.replace(/\*\*/g, '')}</h2>
                }
                if (para.startsWith('**')) {
                  const parts = para.split('**')
                  return (
                    <p key={i} className="text-[16px] text-[rgba(242,238,255,0.70)] leading-[1.85]">
                      {parts.map((p, j) => j % 2 === 1 ? <strong key={j} className="text-[#F2EEFF] font-semibold">{p}</strong> : p)}
                    </p>
                  )
                }
                return <p key={i} className="text-[16px] text-[rgba(242,238,255,0.70)] leading-[1.85]">{para}</p>
              })}
            </div>
          </div>
        </div>
      ...
      <section className="bg-[#110828] border-t border-[rgba(192,132,252,0.08)] px-[clamp(24px,5vw,80px)] py-24 text-center">
        <div className="max-w-[700px] mx-auto">
          <h2 className="font-serif font-bold text-[#F2EEFF] mb-4" style={{ fontSize: 'clamp(24px,3vw,40px)' }}>
